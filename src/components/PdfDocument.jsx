...
export const PdfDocument = ({ data }) => {
  ...
  const dateEffetStr = formatDate(dateEffet);
  const dateEcheanceStr = formatDate(dateEcheance);

  // Numéro unique client : DM-YYYYMMDD-XXXX
  const pad = (n) => String(n).padStart(2, '0');
  const datePart = `${dateEffet.getFullYear()}${pad(dateEffet.getMonth() + 1)}${pad(dateEffet.getDate())}`;
    ...
    <Document>
      ...
      <Page size="A4" style={styles.page} wrap>
        ...
        <View break>
          ...
          <View style={styles.bulletPoint}>
            <Text style={styles.bullet}>•</Text>
            <Text style={styles.text}>Date de prise d'effet : <Text style={styles.textBold}>{dateEffetStr}</Text></Text>
          </View>
          <View style={styles.bulletPoint}>
            <Text style={styles.bullet}>•</Text>
            <Text style={styles.text}>Date d'échéance initiale : <Text style={styles.textBold}>{dateEcheanceStr}</Text></Text>
          </View>
